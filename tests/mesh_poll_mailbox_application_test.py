...
from tests.mesh_testing_common import (
    MeshTestCase,
    MeshTestingCommon,
)
...
class TestMeshPollMailboxApplication(MeshTestCase):
    ...
    def test_mesh_poll_mailbox_happy_path(self, mock_response):
        """Test the lambda"""
        mailbox_id = MeshTestingCommon.TEST_MAILBOX_ID1

        # Mock response from MESH server
        mock_response.get(
            f"/messageexchange/{mailbox_id}/inbox",
            text=json.dumps(
                {
                    "messages": [
        ...
        self.assertEqual(
            mailbox_id, response["body"]["message_list"][0]["body"]["dest_mailbox"]
        )
